...
//!
//! RFC 3986's unreserved set and nothing more, which is what the Shared
//! Key's canonicalized resource demands: `a b` is `a%20b` and never
//! `a+b`, and a `/` in a name is a path separator only where the caller
//! says it is.
